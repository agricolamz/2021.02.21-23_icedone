...
```{block, type = "rmdtask"}
Скачайте все произведения Гертруды Стайн (по-английски, она Gertrude Stein) из базы данных и вставьте название самого длинного текста.
```

```{r stein, echo=FALSE, results='asis', cache=TRUE}
gutenberg_metadata %>% 
  filter(str_detect(author, "Stein, Gertrude"),
         language == "en") %>% 
  pull(gutenberg_id) %>% 
...
```{block, type = "rmdtask"}
Скачайте все произведения Гертруды Стайн из базы данных, уберите стопслова и визуализируйте топ 20 слов из каждого текста.
```
...
```{block, type = "rmdtask"}
Скачайте произведение Гертруды Стайн "Geography and Plays" и визуализируйте распределение в тексте слов time и living.
```

```{r, message=FALSE, echo=FALSE}
stein %>% 
  filter(title == "Geography and Plays") %>% 
  unnest_tokens(word, text) %>% 
  mutate(narrative_time = 1:n()) %>% 
  filter(str_detect(word, "^time$|living$")) %>%  
  ggplot()+
      geom_vline(aes(xintercept = narrative_time))+
  facet_wrap(~word, ncol = 1)
```
